...
static RES: (f64, f64) = (1280.0, 800.0);
...
pub fn detect_loop_hybrid(cam_index: i32, preview: bool, preview_hz: f32) -> Result<()> {
    ...
    let mut frame_num = 0;
    let mut first = true;
    let mut last_preview_dt = Instant::now();
    let (tx, rx) = channel();
    thread::spawn(move || {
        let tx = tx.clone();
        loop {
            ...
            if cam.read(&mut frame).is_err() {
                ...
                if let Ok(v) = videoio::VideoCapture::new(cam_index, videoio::CAP_ANY) {
                    cam = v;
                }
                let _ = cam.set(
                    videoio::CAP_PROP_FOURCC,
    ...
    if preview {
        let _ = highgui::named_window(&format!("seancv{cam_index}"), cam_index);
    }
    while let Ok(mut frame) = rx.recv() {
        // distribute to thread pool here
        let Ok(frame_img) = frame.to_image() else {
            continue;
        };
        let calc_time = Instant::now();
        //let mut detectable_image = Image::zeros_with_alignment(RES.0 as usize, RES.1 as usize, 96).unwrap();

        // for dx in 0..(RES.0 as usize) {
        //     for dy in 0..(RES.1 as usize) {
        //         detectable_image[(dx, dy)] = frame_img.as_rgb8().unwrap().as_bytes()[dx * dy + dy];
        //     }
        // }
        // let detections = detector.detect(&detectable_image);
        //let mut image_raw_buf = Image::new_uinit(RES.0 as usize, RES.1 as usize).unwrap();
        //std::mem::swap(&mut image_raw_buf.as_slice_mut().to_vec(), &mut frame_img.as_rgb8().unwrap().to_vec());
        let detections = detector.detect(&Image::from_image_buffer(&frame_img.to_luma8()));
        frame_num += 1;
        ...
        for det in detections {
            ...
            if let Some(pose) = pose {
                ...
                // cam id
                compute_offset(cam_index as usize, det.id(), x, z, y);
                if preview {
        ...
        if preview && last_preview_dt.elapsed() >= Duration::from_millis((1000.0 / preview_hz) as u64) {
            thread::spawn(move || {
                println!("frame");
                highgui::imshow(&format!("seancv{cam_index}"), &frame)?;
                last_preview_dt = Instant::now();
